...
import SparkPerpetual, { Asset, OrderType } from "../src";

import {
  DEFAULT_AMOUNT,
  DEFAULT_TOKEN,
  initializeSparkPerpetual,
} from "./utils";
...
async function main() {
  ...
  const spark = await initializeSparkPerpetual(wallet);
...
function mint(spark: SparkPerpetual, token: Asset, amount: string) {
  return spark.mintToken(token, amount);
}

function deposit(spark: SparkPerpetual, token: Asset, amount: string) {
  return spark.deposit(token, amount);
}

function createOrder(
  spark: SparkPerpetual,
  amount: string,
  price: string,
  type: OrderType,
) {
...
function cancelOrder(spark: SparkPerpetual, orderId: string) {
